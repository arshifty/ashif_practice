...
export class EditCustomerComponent implements OnInit {
  ...
  onFormControlChange() {
    ...
    this.customerUpdateForm.get("defer_all_fees_except_sms").valueChanges.subscribe(x => {
      if (x) {
        this.customerUpdateForm.controls['waive_all_fees_incld_sms'].setValue(false);

        this.customerUpdateForm.controls['waive_others_fees_from'].setValidators([Validators.required]);
        this.customerUpdateForm.controls['waive_others_fees_from'].updateValueAndValidity();
        this.customerUpdateForm.controls['waive_others_fees_to'].setValidators([Validators.required]);
        this.customerUpdateForm.controls['waive_others_fees_to'].updateValueAndValidity();
      } else {
        this.customerUpdateForm.controls['waive_others_fees_from'].clearValidators();
        this.customerUpdateForm.controls['waive_others_fees_to'].clearValidators();
        this.customerUpdateForm.controls['waive_others_fees_to'].updateValueAndValidity();
        this.customerUpdateForm.controls['waive_others_fees_from'].updateValueAndValidity();
      }
    })

    this.customerUpdateForm.get("waive_all_fees_incld_sms").valueChanges.subscribe(x => {
      if (x) {
        this.customerUpdateForm.controls['defer_all_fees_except_sms'].setValue(false);
        this.customerUpdateForm.controls['waive_all_fees_from'].setValidators([Validators.required]);
        this.customerUpdateForm.controls['waive_all_fees_from'].updateValueAndValidity();
        this.customerUpdateForm.controls['waive_all_fees_to'].setValidators([Validators.required]);
        this.customerUpdateForm.controls['waive_all_fees_to'].updateValueAndValidity();
      } else {
        this.customerUpdateForm.controls['waive_all_fees_from'].clearValidators();
        this.customerUpdateForm.controls['waive_all_fees_to'].clearValidators();
        this.customerUpdateForm.controls['waive_all_fees_to'].updateValueAndValidity();
        this.customerUpdateForm.controls['waive_all_fees_from'].updateValueAndValidity();
      }
    })
  ...
  getCustomer(id) {
    this._customerService.getCustomer(id).subscribe((result: any) => {
      if (result.success) {
        ...
        this._customerService.getCoutiesArea(this.customer.county_id).subscribe((resultArray: any) => {

          this.counties = resultArray[0].data;
          this.areas = resultArray[1].data;

          this.customerUpdateForm.controls['org_name'].setValue(this.customer.org_name);
          this.customerUpdateForm.controls['address1'].setValue(this.customer.address1);
          this.customerUpdateForm.controls['address2'].setValue(this.customer.address2);
          this.customerUpdateForm.controls['county_id'].setValue(this.customer.county_id);
          this.customerUpdateForm.controls['area_ids'].setValue(this.customer.area_ids);
          this.customerUpdateForm.controls['postcode'].setValue(this.customer.postcode);
          this.customerUpdateForm.controls['number_of_member'].setValue(this.customer.number_of_member);
          this.customerUpdateForm.controls['org_type'].setValue(this.customer.org_type);

          if (this.customer.color_1)
            this.customerUpdateForm.controls['color_1'].setValue(this.customer.color_1);
          else
            this.customer.controls['color_1'].setValue("#ffffff");
          if (this.customer.color_2)
            this.customerUpdateForm.controls['color_2'].setValue(this.customer.color_2);
          else
          ...
          if (this.customer.lotto_template_image) {
            this.lotto_template_image_url = this._customerService.imageUrl + this.customer.lotto_template_image + `?${new Date().getTime()}`;
          }

          if (this.customer.prize_template_image) {
            this.prize_template_image_url = this._customerService.imageUrl + this.customer.prize_template_image + `?${new Date().getTime()}`;
          }

          if (this.customer.fifty_template_image) {
            this.fifty_template_image_url = this._customerService.imageUrl + this.customer.fifty_template_image + `?${new Date().getTime()}`;
          }
          if (this.customer.lotto_widget_image) {
            this.lotto_widget_image_url = this._customerService.imageUrl + this.customer.lotto_widget_image + `?${new Date().getTime()}`;
          }
          if (this.customer.prize_widget_image) {
            this.prize_widget_image_url = this._customerService.imageUrl + this.customer.prize_widget_image + `?${new Date().getTime()}`;
          }
          if (this.customer.fifty_widget_image) {
            this.fifty_widget_image_url = this._customerService.imageUrl + this.customer.fifty_widget_image + `?${new Date().getTime()}`;
          }


          this.customerUpdateForm.controls['vendor_name'].setValue(this.customer.payment.vendor_name);
          this.customerUpdateForm.controls['integration_key'].setValue(this.customer.payment.integration_key);
          this.customerUpdateForm.controls['integration_password'].setValue(this.customer.payment.integration_password);
          this.customerUpdateForm.controls['is_active'].setValue(!this.customer.account_status.is_active);

          //Financial
          this.customerUpdateForm.controls['defer_all_fees_except_sms'].setValue(this.customer.defer_all_fees_except_sms);
          this.customerUpdateForm.controls['waive_all_fees_incld_sms'].setValue(this.customer.waive_all_fees_incld_sms);

          this.customerUpdateForm.controls['waive_others_fees_from'].setValue(this.customer.waive_others_fees_from);
          this.customerUpdateForm.controls['waive_others_fees_to'].setValue(this.customer.waive_others_fees_to);
          this.customerUpdateForm.controls['waive_all_fees_from'].setValue(this.customer.waive_all_fees_from);
          this.customerUpdateForm.controls['waive_all_fees_to'].setValue(this.customer.waive_all_fees_to);



          if (this.customer.supporting_teams && this.customer.supporting_teams.length) {
            this.team_list = this.customer.supporting_teams.map(x => { return x.team_name })
          }

          //Marketing
          this.customerUpdateForm.controls['established_in'].setValue(this.customer.established_in);
          this.customerUpdateForm.controls['established_for'].setValue(this.customer.established_for);
          this.customerUpdateForm.controls['org_contribution_detail'].setValue(this.customer.org_contribution_detail);
          this.customerUpdateForm.controls['website'].setValue(this.customer.website);
          this.customerUpdateForm.controls['facebook_page'].setValue(this.customer.facebook_page);
          this.customerUpdateForm.controls['twitter_id'].setValue(this.customer.twitter_id);
          this.customerUpdateForm.controls['instagram'].setValue(this.customer.instagram);
          this.customerUpdateForm.controls['number_of_registered_member'].setValue(this.customer.number_of_registered_member);
          this.customerUpdateForm.controls['number_of_player'].setValue(this.customer.number_of_player);
          this.customerUpdateForm.controls['number_of_trainers'].setValue(this.customer.number_of_trainers);
          this.customerUpdateForm.controls['number_of_teams'].setValue(this.customer.number_of_teams);
          this.customerUpdateForm.controls['number_of_volunteers'].setValue(this.customer.number_of_volunteers);
          this.customerUpdateForm.controls['number_of_winners'].setValue(this.customer.number_of_winners);
          ...
          this.customerUpdateForm.controls['first_name_primary'].setValue(primary.first_name);
          this.customerUpdateForm.controls['last_name_primary'].setValue(primary.last_name);
          this.customerUpdateForm.controls['phone_primary'].setValue(primary.phone);
          this.customerUpdateForm.controls['phone_code_primary'].setValue(primary.phone_code);
          if ((primary.phone_code == '+353' || primary.phone_code == '353') && primary.phone && primary.phone.length) {

            let first_two_digit = primary.phone.slice(0, 2);
            if (first_two_digit == '08' && primary.phone.length == 10) {
              let phone = primary.phone.slice(2, primary.phone.lenght);
              this.customerUpdateForm.controls['phone_primary'].setValue(phone);
            }
            else if (primary.phone[0] == '8' && primary.phone.length == 9) {
              let phone = primary.phone.slice(1, primary.phone.lenght);
              this.customerUpdateForm.controls['phone_primary'].setValue(phone);
            }
          } else {
            let phone = primary.phone[0] == '0' ? primary.phone.slice(1, primary.phone.lenght) : primary.phone;
            this.customerUpdateForm.controls['phone_primary'].setValue(phone);
          }


          this.customerUpdateForm.controls['role_primary'].setValue(primary.role_id);
          this.customerUpdateForm.controls['email_primary'].setValue(primary.email);
          if (!primary.password_created)
            this.resend_primary_invite = true
          this.bill_id = bill._id;
          this.customerUpdateForm.controls['first_name_bill'].setValue(bill.first_name);
          this.customerUpdateForm.controls['last_name_bill'].setValue(bill.last_name);
          this.customerUpdateForm.controls['phone_bill'].setValue(bill.phone);
          this.customerUpdateForm.controls['phone_code_bill'].setValue(bill.phone_code);

          if ((bill.phone_code == '+353' || bill.phone_code == '353') && bill.phone && bill.phone.length) {
            let first_two_digit = bill.phone.slice(0, 2);
            if (first_two_digit == '08' && bill.phone.length == 10) {
              let phone = bill.phone.slice(2, bill.phone.lenght);
              this.customerUpdateForm.controls['phone_bill'].setValue(phone);
            }
            else if (bill.phone[0] == '8' && bill.phone.length == 9) {
              let phone = bill.phone.slice(1, bill.phone.lenght);
              this.customerUpdateForm.controls['phone_bill'].setValue(phone);
            }
          } else {
            let phone = bill.phone[0] == '0' ? bill.phone.slice(1, bill.phone.lenght) : bill.phone;
            this.customerUpdateForm.controls['phone_bill'].setValue(phone);
          }
          ...
          if (!bill.password_created)
            this.resend_bill_invite = true
  ...
  update() {
    ...
    if (this.customerUpdateForm.valid) {
      ...
      let data: any = {
        values: this.customerUpdateForm.value,
        id: this.customer_id,
        primary_id: this.primary_id,
        bill_id: this.bill_id,
        logo: this.customer.logo,
        lotto_widget_image: this.customer.lotto_widget_image,
        prize_widget_image: this.customer.prize_widget_image,
        fifty_widget_image: this.customer.fifty_widget_image,
        lotto_template_image: this.customer.lotto_template_image,
        prize_template_image: this.customer.prize_template_image,
        fifty_template_image: this.customer.fifty_template_image
      }

      if (data.values.phone_primary && data.values.phone_primary.toString().length) {
        if (data.values['phone_code_primary'] == '+353' || data.values['phone_code_primary'] == '353') {
          data.values["phone_primary"] = '08' + data.values["phone_primary"];
        }
        data.values.phone_primary = data.values.phone_primary.toString()[0] == "0" ? data.values.phone_primary.toString() : "0" + data.values.phone_primary.toString();
      }

      if (data.values.phone_bill && data.values.phone_bill.toString().length) {
        if (data.values['phone_code_bill'] == '+353' || data.values['phone_code_bill'] == '353') {
          data.values["phone_bill"] = '08' + data.values["phone_bill"];
        }
        data.values.phone_bill = data.values.phone_bill.toString()[0] == "0" ? data.values.phone_bill.toString() : "0" + data.values.phone_bill.toString();
      }

      data.values.county = this.counties.find(cnty => cnty._id == data.values.county_id).name;
      data.values.areas = this.areas.filter(ara => data.values.area_ids.indexOf(ara._id) > -1).map(slar => { return slar.name });
      data.values.supporting_teams = this.team_list.map((x, i) => { return { team_name: x, seq: (i + 1) } });
  ...
  downloadTemplate() {
    this._excelService.exportAsExcelFile([{
      email: 'example@email.com', phone: '', number_of_game_remaining: 5, ticket_1: '12,23,28,32', ticket_2: '7,15,19,29', ticket_3: '5,17,23,27', ticket_4: '', ticket_5: ''
    }, {
      email: '', phone: '0834874157', number_of_game_remaining: 4, ticket_1: '5,13,16,30', ticket_2: '11,15,19,29', ticket_3: '22,26,29,30', ticket_4: '', ticket_5: ''
    }, {
      email: 'player@email.com', phone: '', number_of_game_remaining: 9, ticket_1: '7,20,22,31', ticket_2: '5,9,16,18', ticket_3: '1,3,8,17', ticket_4: '9,10,17,26', ticket_5: '4,14,24,26'
    }], `sales`);
  }
